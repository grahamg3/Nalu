...
InflowPerturbationAuxFunction::InflowPerturbationAuxFunction(
  ...
  az_(0.1)
{
  ...
  if ( params.size() != 6 || params.empty() )
    throw std::runtime_error("InflowPerturbationAuxFunction: requires u, v, w, ax, ay, az"); 
  ux_ = params[0];
  uy_ = params[1];
  uz_ = params[2];
  ax_ = params[3];
...
void
InflowPerturbationAuxFunction::do_evaluate(
  const double *coords,
  const double time,
