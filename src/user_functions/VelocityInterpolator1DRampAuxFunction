...
VelocityInterpolator1DRampAuxFunction::VelocityInterpolator1DRampAuxFunction(
  ...
  params_(params)
{
  ...
  if ( params.empty() )
    throw std::runtime_error("VelocityInterpolator1DRampAuxFunction: requires direction, default velocity components, table"); 
...
void
VelocityInterpolator1DRampAuxFunction::do_evaluate(
  const double *coords,
  const double time,
